...
public class SLinkedList<E extends DeepClone<E>> implements Iterable<E> {
  ...
  *  Hint: Use E.deepCopy(). (not deepClone??)
  */
 
 
 //copied from class
 	public SLinkedList<E> deepClone() { //runtime: O(n)
 		
 		
 		SLinkedList<E> cloneList = new SLinkedList<E>();	//creates a new clone of the linked list
 		SNode<E> peeker = this.head;			//temporary first node is the head of THIS list, called it peeker
 		//this will be used to see through *this* list
 		
 		//EDGE CASE: if the list is empty...
 		if(this.head==null)
 			return cloneList;	//return a null linked list
 		
 		else {
	 		//a clone node is created from the original list to be the head of the cloneList
	 		cloneList.head = new SNode<E>(this.head.element.deepClone());
	 		
	 		//now iterating through rest of list
	 		SNode<E> cloneNode = cloneList.head;	//we set a new variable, cloneNode, as the head of the cloned list
	 		cloneList.size++;	//logical size is updated
	 		
	 		//while loop iterates until tail is reached
	 		while(!peeker.equals(this.tail)) {	//while the peeker node is not the tail of this linked list
	 			peeker=peeker.next;	//go to the next node in the list
	 			cloneNode.next = new SNode<E>(peeker.element.deepClone());	//the head now points to the second node that is
	 																//in the original linked list. term is copied
	 			cloneNode = cloneNode.next; //reset to next node
	 			cloneList.size++; 		//logical size is updated
	 		}
	 		
	 		//when done with the while loop, tail is added
	 		cloneList.tail = cloneNode; //as cloneNode was updated to next in the while loop
	 		//returns cloned list - done
	 		return cloneList;
	 	}
